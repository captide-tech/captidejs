import { findBestScrollTarget } from './highlighting';
import { extractPageNumberFromElementId, isProxyStatement } from './page-utils';
...
export const handlePageBasedDocumentLoad = (
  ...
  if ((document.formType?.toUpperCase() === '8-K' || isProxyStatement(document.formType)) && highlightedElementId) {
    ...
    if (isProxyStatement(document.formType)) {
  ...
  if (highlightedElementId) {
    ...
    setTimeout(() => {
      ...
      if (elementsToHighlight.length > 0) {
        ...
        if (document.formType?.toUpperCase() === '10-K' || document.formType?.toUpperCase() === '10-Q') {
        ...
        const scrollTarget = findBestScrollTarget(elementsToHighlight, document.formType);
        
        if (scrollTarget) {
          handleElementScrolling(iframe, scrollTarget, document.formType, isNewDocument);
        }
      }
    }, delay);
  }
 ...
 * @param formType Document form type
 * @param isNewDocument Whether this is a new document
 */
export const handleElementScrolling = (
  ...
  if (formType?.toLowerCase() === 'transcript') {
    handleTranscriptScrolling(iframe, scrollTarget, isNewDocument);
  } else if (formType?.toUpperCase() === '10-K' || formType?.toUpperCase() === '10-Q') {
